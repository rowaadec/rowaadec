( function($) {
  'use strict';
    ...
    $('.js-target-scroll, .navbar-desktop-menu li a').on('click', function() {
        var target = $(this.hash);
        if (target.length) {
            $('html,body').animate({
                scrollTop: (target.offset().top - navbarDesktopHeight + 1)
            }, 1000);
            return false;
        }
    });
